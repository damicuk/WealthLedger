...
AssetTracker.prototype.ukChartsDataSheet = function (sheetName = this.ukChartsDataSheetName) {
  ...
  if (this.getSheetVersion(sheet) !== version) {
    ...
    const formulas = [[
      ...
      `IF(COUNT(QUERY(${referenceRangeName2}, "SELECT P WHERE O='Trade'"))=0,,QUERY(${referenceRangeName2}, "SELECT G, SUM(T), SUM(U) WHERE O='Trade' GROUP BY G ORDER BY G LABEL SUM(T) '', SUM(U) ''"))`, , ,
      `IF(COUNT(QUERY(${referenceRangeName2}, "SELECT P WHERE O='Trade'"))=0,,QUERY(${referenceRangeName2}, "SELECT G, F, SUM(T), SUM(U) WHERE O='Trade' GROUP BY G, F ORDER BY G, F LABEL SUM(T) '', SUM(U) ''"))`, , , ,
      `IF(COUNT(QUERY(${referenceRangeName2}, "SELECT P WHERE O='Trade'"))=0,,QUERY(${referenceRangeName2}, "SELECT YEAR(J), SUM(T), SUM(U) WHERE O='Trade' AND YEAR(J)>"&YEAR(TODAY())-5&" GROUP BY YEAR(J) ORDER BY YEAR(J) LABEL YEAR(J) '', SUM(T) '', SUM(U) ''"))`
    ]];
